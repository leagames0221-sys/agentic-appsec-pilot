...
Accepted (2026-05-21) — **Revised same day** to align the verification protocol with the author's current AI-assisted-development workflow: writer-driven verification with literal evidence citation, no second-AI dispatch dependency. See Condition 5 + Sequence below for the revised path.
...
### Condition 5: Writer self-verification with literal evidence + portfolio-quality basics

**Revised 2026-05-21**: protocol is writer-driven verification with
literal evidence citation. The author maintains a portfolio-quality
basics checklist out-of-tree; the in-tree rubric below is the
project-specific verification.

- The author applies the 7-criteria Phase α rubric
...
- The author additionally applies a cross-project portfolio-quality
  basics checklist (private, maintained out-of-tree); all items must
  PASS. One FAIL drops the verdict to ★★ or lower.
- Cross-verification path (open to user / human reviewer):
  re-run the literal verify commands cited per criterion against the
  same HEAD, re-apply the basics checklist, and compare verdicts. Any
  divergence is a regression and blocks PUBLIC.
- Optional human review: the user MAY request a fresh independent
  reviewer to re-apply both rubrics. This is optional, not blocking.

Rationale: verification with literal evidence citation keeps
accountability with the author and produces reproducible verdicts
that a third party can independently re-run.
...
[Conditions 1-4 green]
       ↓
[Author applies 7-criteria Phase α rubric, records verdict file
 docs/verify/phase-alpha-round-1-self-verify.md with literal evidence
 paths + `?` honesty markers]
       ↓
[Author applies portfolio-quality basics checklist; all items PASS
 required; one FAIL → ★★ or lower, fix and re-apply until clean]
       ↓
[Author fixes any latent issues caught by either rubric, regression
...
[User issues explicit promotion sentence ("PUBLIC flip OK" or
 unambiguous equivalent — closure-bias defense]
       ↓
...
steps above (a fresh reviewer re-running the literal verify commands
+ re-applying the basics checklist); recommended but not blocking,
since primary verification responsibility rests with the author.
...
2. **Companion-repo pattern**: [sbom-pilot](https://github.com/leagames0221-sys/sbom-pilot)
   shipped via multi-round verification + user-explicit-gate. This ADR
   replays the same shape adapted to the current author-driven
...
- Verdict precedent: [sbom-pilot](https://github.com/leagames0221-sys/sbom-pilot) (PUBLIC, 2026-05-20)
- Portfolio-quality basics checklist (private, maintained out-of-tree by the author)
- closure-bias defense rationale (author's private governance notes)
